...
import { listarCategorias, Noticia } from '../services/api';

const Categoria: React.FC = () => {
  const { filtrarNoticias, loading } = useNoticias();
  const [busca, setBusca] = useState('');
  const [categoriaAtiva, setCategoriaAtiva] = useState('Todos');
  const [campusAtivo, setCampusAtivo] = useState('Todos');
  const [itensVisiveis, setItensVisiveis] = useState(9);

  const categorias = listarCategorias();
  const campi = ['Todos', 'Benfica', 'Pici', 'Porangabuçu', 'Sobral', 'Quixadá', 'Crateús', 'Russas', 'Itapajé'];

  const noticiasFiltradas = filtrarNoticias(categoriaAtiva, busca);
  const noticiasFinais = campusAtivo === 'Todos' 
    ? noticiasFiltradas 
    : noticiasFiltradas.filter((n: Noticia) => n.campus === campusAtivo);
  const noticiasExibidas = noticiasFinais.slice(0, itensVisiveis);
    ...
    <main className="container mx-auto px-6 py-8 space-y-10">
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 space-y-6">
        <div className="flex flex-col md:flex-row gap-4">
          ...
          >
            {campi.map((c) => (
              <option key={c} value={c}>{c === 'Todos' ? 'Todos os Campi' : `Campus ${c}`}</option>
            ))}
          </select>
        </div>
      ...
      {loading ? (
        ...
        <>
          {noticiasExibidas.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8" data-testid="lista-noticias">
          ...
          {noticiasFinais.length > itensVisiveis && (
